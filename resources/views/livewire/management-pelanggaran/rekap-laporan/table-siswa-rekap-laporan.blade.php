    ...
    <div class="overflow-x-auto">
        <table class="whitespace-nowrap min-w-full border border-gray-200 rounded-xl overflow-hidden">
            ...
            <tbody id="siswaTable" class="divide-y divide-gray-100">
                @foreach ($recapPerStudent as $i => $item)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="py-3 px-4 text-center">{{ $i + 1 }}</td>
                        <td class="py-3 px-4 text-center font-medium text-gray-800">{{ $item['nama_siswa'] }}</td>
                        <td class="py-3 px-4 text-center text-gray-700">{{ $item['kelas'] }}</td>
                        <td class="py-3 px-4 text-center text-blue-600 font-semibold">{{ $item['total_pelanggaran'] }}</td>
